...
/* เดาแบบ "ฟันธง" เอาไว้ตั้งซับให้อัตโนมัติ — ใช้ได้เฉพาะตอนที่ผู้ต้องสงสัยอันดับหนึ่งไม่มีใครเสมอกัน
   (เช่น ทะเบียนนี้เคยรับเคลม/ตั้งซับสัมปทานไว้ค่าเดียวชัดเจน) ถ้าเสมอกันหลายราย (เช่น อยู่ในรายชื่อรถของ
   หลายซับพร้อมกัน) ถือว่ายังไม่ฟันธง คืน null — ให้ไปเดาแบบ "ผู้ต้องสงสัย" (guessOwner) แทน ห้ามเดาแทนคน */
function bestGuess(c){
  const g = guessOwner(c);
  if(!g.length) return null;
  if(g[1] && g[1].tier === g[0].tier) return null;
  return g[0];
}
